...
bias = tf.Variable(initial_value=0.1, dtype=tf.float32, name="b")
#model = tf.add(tf.multiply(tf_data_x, weight), bias)
model = tf.sin(tf.multiply(weight, tf_data_x))
model = tf.add(bias, model)
...
optimizer = tf.train.AdadeltaOptimizer(learning_rate=0.5).minimize(loss)  # метод оптимизации, о нём тоже ниже

for _ in range(100):
    with tf.Session() as session:
        tf.global_variables_initializer().run()
        for i in range(samples//packetSize): #samples//packetSize
        ...
        plt.plot(not_shuffled, list(map(lambda x: bias.eval() + math.sin(weight.eval() * x), not_shuffled)), data_x, data_y, 'ro')
plt.show()
